...
void ahrs_task(void *arg) {
	...
	while (true) {
		...
		|         0|    TRUE     |    TRUE    |     1MHz| 965| 129 %|
		|         0|    FALSE    |    FALSE   |     1MHz|1040| 139 %|
...
void parent_task(void *arg) {
	I2CMaster *i2c = new I2CMaster(&ESPIDF::Grove_M5Stick);
...
void app_main(void) {
	...
	// BluetoothはCore0
	xTaskCreatePinnedToCore(ble_send_task, "hid_task", 2048, NULL, 5, NULL, 0);

	I2CMaster *i2c = new I2CMaster(&ESPIDF::Default_M5Stick);
